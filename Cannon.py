...
class Cannon(Piece):
    ...
    def is_valid_move(self, board, curr, dest):
        ...
        if board.is_in_palace(curr) and not board.is_in_palace(dest) and abs(x2 - x1) > 0 and abs(y2 - y1) > 0:
            return False
        # jump palaces
        if board.is_in_blue_palace(curr) and board.is_in_red_palace(dest) and abs(x2 - x1) > 0 and abs(y2 - y1) > 0:
            return False
        if board.is_in_red_palace(curr) and board.is_in_blue_palace(dest) and abs(x2 - x1) > 0 and abs(y2 - y1) > 0:
            return False
            ...
            if x2 - x1 > 0:  # move right
                ...
                while not is_blocked and (x1 + i) > x2:
                    if board.get_board()[y2][x1 + i] != '__':
                        ...
                        unit_count += 1
                    i -= 1
                if unit_count < 1 or unit_count > 1:
                    is_blocked = True
